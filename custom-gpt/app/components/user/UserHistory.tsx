...
const ConversationItem = memo(({ conv, formatTimestamp, onDelete, isDarkMode, navigate }: {
  ...
  navigate: (path: string, options?: any) => void;
    ...
    <div
        ...
        onClick={() => navigate(`/user/chat/${conv.gptId}?loadHistory=true`, {
...
const UserHistoryPage: React.FC<UserHistoryProps> = ({ initialData }) => {
    // Add safety checks for initialData
    const { 
        user, 
    ...
    // Fetch conversation history using Remix fetcher
    const fetchConversationHistory = useCallback(async () => {
        setLoading(true);
        setError(null);
        
        // Use fetcher to get conversation history from an API endpoint
        fetcher.load(`/api/conversation-history?userId=${user._id}`);
    }, [user?._id, fetcher]);
    ...
    useEffect(() => {
        if (fetcher.data) {
            if ((fetcher.data as any).success) {
                const formattedConversations = (fetcher.data as any).conversations.map((conv: any) => ({
                    id: conv._id,
                    gptId: conv.gptId,
    ...
    }, [user, fetchConversationHistory, conversations.length]);
    ...
    const handleDeleteConversation = useCallback(async () => {
        ...
        fetcher.submit(
            ...
            { method: 'POST', action: '/api/conversation-history' }
        );
